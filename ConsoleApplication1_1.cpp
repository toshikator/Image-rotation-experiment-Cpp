...
bool finish(const char* message)//выход из программы в случае ошибки
{
	std::cerr << message << std::endl;
	exit(0);
}

constexpr uint16_t get16bit(char buffer[])//сцепка в двубайтный
{
	return
		static_cast<uint16_t>(static_cast<uint8_t>(buffer[0])) |
		static_cast<uint16_t>(static_cast<uint8_t>(buffer[1])) << 8;
}
constexpr uint32_t get32bit(char buffer[])//сцепка в четырехбайтный
{
	return
...
void set16bit(uint16_t data, char buffer[])//заполнение буфера двубайтного
{
	buffer[0] = static_cast<char>(data);
	buffer[1] = static_cast<char>(data >> 8);
}
void set32bit(uint32_t data, char buffer[])//заполнение буфера четырехбайтного
{
	buffer[0] = static_cast<char>(data);
	buffer[1] = static_cast<char>(data >> 8);
...
//это бекап, ниже я его изнасилую
RGBAdata A11applyBlurMatrix(const RGBAdata& data, int8_t mat[3][3])//создаем размытие на изображении
{
	RGBAdata res = data;
...
RGBAdata applyBlurMatrix(const RGBAdata& data, int angle, uint32_t pixelWidth,uint32_t pixelHeight)//создаем размытие на изображении
{
	uint16_t pointOx = pixelWidth/2;
	uint16_t pointOy = pixelHeight/2;
	RGBAdata before = data;
	RGBAdata after;
	auto RadAngle = (PI/180)*angle*-1;
	auto offsetSin = sin(RadAngle);
	auto offsetCos = cos(RadAngle);

	//инициализация пустотой, т.к. часть картины пропадет из видимости
	for (size_t row = 1, endRow = data.size() - 1; row < endRow; ++row)
	{
		for (size_t col = 1, endCol = data[row].size() - 1; col < endCol; ++col)
		{
	...
	//подбор координат для пикселей при вращении на заданный угол   
	// xNEW=x*cosFI-y*sinFI
	// yNEW=x*sinFI+y*cosFI
	for (size_t row = 1, endRow = data.size() - 1; row < endRow; ++row)
	{
		for (size_t col = 1, endCol = data[row].size() - 1; col < endCol; ++col)
		{
			auto x = col - pointOx;
			auto y = row - pointOy;
			after[row][col].r = before[(x * offsetSin + y * offsetCos)][(x * offsetCos - y * offsetSin)].r;
			after[row][col].g = before[(x * offsetSin + y * offsetCos)][(x * offsetCos - y * offsetSin)].g;
			after[row][col].b = before[(x * offsetSin + y * offsetCos)][(x * offsetCos - y * offsetSin)].b;
			after[row][col].a = before[(x * offsetSin + y * offsetCos)][(x * offsetCos - y * offsetSin)].a;

			/*
			after[row][col].g = before[(row * offsetCos - col * offsetSin)][(row * offsetCos + col * offsetSin)].g;
			after[row][col].b = before[(row * offsetCos - col * offsetSin)][(row * offsetCos + col * offsetSin)].b;
			after[row][col].a = before[(row * offsetCos - col * offsetSin)][(row * offsetCos + col * offsetSin)].a;
			*/
		}
	}
...
int main(int argc, char* argv[])
{
	...
	auto blurredData = applyBlurMatrix(data, angle, pixelWidth, pixelHeight);
