...
RGBAdata applyRotateMatrix(const RGBAdata& data, int angle, uint32_t pixelWidth,uint32_t pixelHeight)//создаем размытие на изображении
{
	...
	{
		//std::cout << "строка:" << row << std::endl;
		for (size_t col = 0; col < data[row].size(); col++)
		{
			//std::cout << "столбец:" << col << std::endl;
			...
			*/
			...
			colOLD = x1 - x2;
			/*
			std::cout << "after_строка: " << row << " after_столбец: " << col << " before_строка: " << rowOLD << " before_столбец: " << colOLD << std::endl;
			*/
